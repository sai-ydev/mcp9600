 ...
 * */
err_t MCP9600::init(u8 therm_type, I2C_HandleTypeDef *i2c) {
	err_t ret = NO_ERROR;
	u16 ver;
	IIC_begin(i2c);
...
err_t MCP9600::read_INT_stat(u8 *stat) {
	*stat = 0;
	...
	for (int i = 0; i < 4; i++) {
		if (byte & 1 << i) {
			printf("Interrupt Generated: channel %d\r\n", i);
		}
	}
	*stat = byte;
	return NO_ERROR;
...
err_t MCP9600_IIC_OPRTS::IIC_write_byte(u8 reg, u8 byte) {
	uint8_t ret = 0;
	ret = HAL_I2C_Mem_Write(i2c, _IIC_ADDR, reg, 1, &byte, 1, HAL_MAX_DELAY);
...
err_t MCP9600_IIC_OPRTS::IIC_write_16bit(u8 reg, u16 value) {
	...
	ret = HAL_I2C_Mem_Write(i2c, _IIC_ADDR, reg, 1, data, 2, HAL_MAX_DELAY);
...
err_t MCP9600_IIC_OPRTS::IIC_read_byte(u8 reg, u8 *byte) {
	...
	ret = HAL_I2C_Mem_Read(i2c, _IIC_ADDR, reg, 1, byte, 1, HAL_MAX_DELAY);
	if (!ret) {
		return NO_ERROR;
	} else {
		return ERROR_COMM;
...
err_t MCP9600_IIC_OPRTS::IIC_read_16bit(u8 start_reg, u16 *value) {
	...
	*value = 0;
	...
	ret = HAL_I2C_Mem_Read(i2c, _IIC_ADDR, start_reg, 1, val, 2, HAL_MAX_DELAY);
	*value = (val[0] << 8) | val[1];
...
err_t MCP9600_IIC_OPRTS::IIC_read_bytes(u8 start_reg, u8 *data, u32 data_len) {
	...
	HAL_I2C_Mem_Read(i2c, _IIC_ADDR, start_reg, 1, data, data_len,
	HAL_MAX_DELAY);
